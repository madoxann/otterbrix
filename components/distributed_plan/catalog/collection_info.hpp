...
struct collection_info {
    explicit collection_info(consistent_hashing::node_id_t primary_node, std::reference_wrapper<node_load_t> record_cnt)
        : records(0)
    ...
    std::pair<consistent_hashing::node_id_t, bool> create_document_distributed(components::document::document_ptr doc) {
        auto node =
